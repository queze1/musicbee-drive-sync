...
EXPORTED_PLAYLIST_PATH = pathlib.Path.home().joinpath('Music/Musicbee/Exported Playlists')
...
class Drive(GoogleDrive):
    ...
    def create_folder(self, path_parts: list[str]):
        ...
        for part in remaining_parts:
            next_folder = self.list_file(title=part,
                                         is_folder=True,
                                         parent_folder=current_folder_id)
            if not next_folder:
                next_folder = self.create_file(title=part,
                                               is_folder=True,
                                               parent_folder=current_folder_id)
                next_folder.Upload()
            else:
...
def strip(str_: pathlib.Path):
    return "".join([char for char in str(str_) if char in string.ascii_letters + string.digits])


def get_songs(path: pathlib.Path, logger: logging.Logger = None)\
        -> list[pathlib.Path]:
...
def main(logger):
    ...
    path_to_songs = {}
    playlist_paths = list(chain(MUSICBEE_PLAYLIST_PATH.rglob('*.mbp'),
                                EXPORTED_PLAYLIST_PATH.rglob('*.m3u')))
    ...
    songs_to_delete = [song for song in existing_songs if song['title'] not in all_song_names]
    ...
    # Upload songs
    songs_to_upload = [song for song in all_songs if song.name not in existing_songs_names]
    ...
    for i, playlist in enumerate(existing_playlists):
        ...
        if hashlib.md5(gen_m3u_content(songs).encode()).hexdigest() != playlist['md5Checksum']:
    ...
    playlists_to_upload = [title for title in title_to_path
    ...
    for i, playlist_title in enumerate(playlists_to_upload):
        ...
        file.Upload()
        logger.debug(f"Playlist file {i+1} of {len(playlists_to_upload)} to upload done. "
